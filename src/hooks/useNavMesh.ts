...
export function useNavMesh() {
  ...
  // * Character
  // const [vehicleGeometry] = useState<THREE.ConeGeometry>(
  //   new THREE.ConeGeometry(0.25, 1, 16)
  // )

  // const [vehicleMaterial] = useState<THREE.MeshNormalMaterial>(
  //   new THREE.MeshNormalMaterial()
  // )

  const [vehicleMesh, setVehicleMesh] = useState<THREE.Mesh>()
  // new THREE.Mesh(vehicleGeometry, vehicleMaterial)

  const [loader] = useState<YUKA.NavMeshLoader>(new YUKA.NavMeshLoader())
  ...
  const init = useCallback(
    (nextVehicleMesh: THREE.Mesh): void => {
      ...
      getScene().add(nextVehicleMesh)
      setVehicleMesh(nextVehicleMesh)
      nextVehicleMesh.position.copy(new THREE.Vector3(0, 0, 0))

      // Rerender
      // getRenderer().setPixelRatio(window.devicePixelRatio)
      // getRenderer().setSize(window.innerWidth, window.innerHeight)
      // document.body.appendChild(getRenderer().domElement)

      // Listeners
      //@ts-ignore
      document.addEventListener('mousedown', moveToPoint, false)

      // Navigation mesh
      loader
        .load('/models/prototype/navmesh.gltf')
